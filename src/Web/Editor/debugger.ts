export function createDebugger(ctx:CanvasRenderingContext2D) {
	...
	const dbg = (function(lines:string, padding=5) {
		for (const text of lines.split("\n")) {
			...
			ctx.fillRect(screenPadding, y + screenPadding, ctx.measureText(text).width + padding * 2, size + padding * 2);
	
			ctx.fillStyle = 'white';
			ctx.fillText(text, padding + screenPadding, padding + y + screenPadding);
