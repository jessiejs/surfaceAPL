...
export function drawTile({ctx,src,pos,cam,lazyload,rotationInAppelDegrees,hue,id,superPain,data,drawImg}:{ctx:CanvasRenderingContext2D,src:string,pos:[number,number],cam:Camera,lazyload:(src:string)=>HTMLImageElement|undefined,rotationInAppelDegrees:number,hue:number,id:number,superPain?:boolean,data?:string,drawImg:boolean}) {
	...
	if (img) {
		...
		if (screenSpacePos[0] + screenSpaceSize[0] / 2 < 0 || screenSpacePos[1] + screenSpaceSize[1] / 2 < 0 || screenSpacePos[0] - screenSpaceSize[0] / 2 > cam.resolution[0] || screenSpacePos[1] - screenSpaceSize[1] / 2 > cam.resolution[1]) {
			return
		}
		...
		const offset = (textureOffsets as unknown as Record<string,[number,number] | undefined>)[src.toLowerCase()] ?? [0,0];
		...
		if (drawImg) {
			...
			ctx.rotate(radians);
			ctx.drawImage(img, -screenSpaceSize[0] / 2 + offset[0] * 62 * cam.zoom, -screenSpaceSize[1] / 2 + offset[1] * 62 * cam.zoom, screenSpaceSize[0], screenSpaceSize[1]);
			ctx.restore();
		}
		//}
		...
		if (data) {
			...
			const size = (doRounding ? Math.round : ($:number) => $)(12 * cam.zoom); // it gets rounded anyways, so its better to explicitly do it so we at least lay out correctly
			...
			const bubbleCenterY = screenSpacePos[1] - screenSpaceSize[1] / 2 - bubbleHeight / 2 - (10 * cam.zoom);
			...
			ctx.beginPath();
			ctx.moveTo(bubbleCenterX - arrowUnits, bubbleCenterY + bubbleHeight / 2);
			ctx.lineTo(bubbleCenterX + arrowUnits, bubbleCenterY + bubbleHeight / 2);
			ctx.lineTo(bubbleCenterX, bubbleCenterY + bubbleHeight / 2 + arrowUnits);
			ctx.fill();
