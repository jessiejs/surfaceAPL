...
export type Filter = (options:{level:Level,pos:[number,number],x:number,y:number,tile:Tile}) => Tile | undefined | void;
...
export function clip(filter:Filter,rect:[number,number,number,number]) {
	return function(options) {
		...
		if (options.x >= x && options.x < rightX && options.y >= y && options.y < bottomY) {
	...
	return {
		...
		_(filter:Filter) {
			...
			for (const tile of level.tiles) {
				const result = filter({
					level,
					pos:indexToStandardCoords(index,level.width,level.height),
					x:indexToStandardCoords(index,level.width,level.height)[0],
					y:indexToStandardCoords(index,level.width,level.height)[1],
					tile
				});
				if (result) {
